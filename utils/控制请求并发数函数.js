 ...
 * @LastEditTime : 2023-05-22 12:12:48
...
const reqList = [() => createReq(1), () => createReq(2), () => createReq(3), () => createReq(4), () => createReq(5), () => createReq(6), () => createReq(7)]
...
function limitReq(requestList = [], limit = 3, callback) {
    ...
    for (let i = 0; i < limit; i++) {
        requestList.length && queue.push({ isStart: false, req: requestList.shift() })
    }
    walker()
    function walker() {
        queue.filter(q => q.isStart === false).forEach((item, index) => {
            item.req().then(res => {
                success.push(res)
            }).catch(err => {
                fail.push(err)
